...
public class DefaultBattleGUI : BattleGUI{
	...
	public override void DoFixedUpdate (){
		...
		if (this.isRunning){
			...
			StoryModeProgression.level = this.player1GUI.lifeBar.fillAmount;
	...
	private TouchableControl tc;
	void OnEnable()
	{
		if (PlayerPrefs.GetInt ("FirstTimePlay") == 0) {
			
			SpecialAndPowerMoveCollector.GetInstance ().touchController.enabled = false;
	...
	public void SwitchOfHelp()
	{
		tutorialImage.SetActive (false);
		PlayerPrefs.SetInt ("FirstTimePlay",1);
		SpecialAndPowerMoveCollector.GetInstance ().touchController.enabled = true;

		PlayerPrefs.SetInt ("ShowStarterPack",1);
	}
