...
public class UFE : MonoBehaviour {
	...
	#endregion
	public static bool player1WonLastBattleVersusMode;
	...
	public static void StartStoryMode(float fadeTime){
		...
		if (!PlayerPrefs.HasKey ("firstTime")) {
			...
			//}
			print (PlayerPrefs.GetInt ("oppoIndex"));
					UFE.storyMode.currentGroup = -1;

			UFE.storyMode.currentBattle=PlayerPrefs.GetInt ("currentBattle");
			UFE.storyMode.currentBattleInformation = null;
			//UFE.storyMode.currentBattleInformation.opponentCharacterIndex = PlayerPrefs.GetInt ("oppoIndex");
	...
	public static void StartStoryModeConversationAfterBattleScreen(UFEScreen conversationScreen, float fadeTime){
		UFE._StartStoryModeConversationAfterBattleScreen(conversationScreen, fadeTime/2);
	...
	public static void FireGameEnds(CharacterInfo winner, CharacterInfo loser){
		...
		if (winner == UFE.GetPlayer1 ()) {
			UFE.storyMode.defeatedOpponents.Add (UFE.storyMode.currentBattleInformation.opponentCharacterIndex);
			PlayerPrefs.SetInt ("oppoIndex", UFE.storyMode.currentBattleInformation.opponentCharacterIndex);
			PlayerPrefs.SetInt ("currentBattle", UFE.storyMode.currentBattle);
			PlayerPrefs.SetInt ("levels", PlayerPrefs.GetInt ("levels") + 1);
				PlayerPrefs.Save ();
				PlayerPrefs.SetFloat ("StoryModeStarsOfLevel" + PlayerPrefs.GetInt ("levels").ToString (), StoryModeProgression.level);
			PlayerPrefs.Save ();

			print ("defeated: " +	UFE.storyMode.defeatedOpponents.Count);

			PlayerPrefs.SetInt ("defeatedoppo", (UFE.storyMode.defeatedOpponents.Count));
			PlayerPrefs.Save ();
			UFE.EndGame ();
			UFE.HideScreen (UFE.currentScreen);
			UFE.StartStoryModeConversationAfterBattleScreen (UFE.storyMode.currentBattleInformation.conversationAfterBattle, .5f);
		...
		if (pause){
			...
			if (Application.platform == RuntimePlatform.Android  || Application.platform == RuntimePlatform.IPhonePlayer) {
				...
				try{
					...
					AdsManager.GetInstance().ShowGamePauseAd();
				}
	...
	public static void HostGame(){
		if (UFE.isNetworkAddonInstalled){
			Network.InitializeServer(2, UFE.config.networkOptions.port, true);
		}
	}
	...
	protected void Awake(){
		...
		if (UFE.config.debugOptions.startGameImmediately){
			...
			if(PlayerPrefs.HasKey("currentBattle"))
				print(PlayerPrefs.GetInt ("currentBattle"));
		
			//bossManager.showBossEnemy = true;
		

				UFE.StartIntroScreen(0f);
		}
	}
	public static bool firstBattle = true;
	...
	public static void _StartStoryModeBattle(float fadeTime){
		...
		if (UFE.player1WonLastBattle){
			print ("inside _start player1won");
			...
			if (UFE.storyMode.currentGroup < 0){
				...
				UFE.storyMode.currentBattle=(PlayerPrefs.GetInt ("levels"));
				print ("current battle: "+PlayerPrefs.GetInt ("levels")+ "  :"+ PlayerPrefs.GetInt ("currentBattle"));
				//				if (PlayerPrefs.HasKey ("currentBattle"))
				//					UFE.storyMode.currentBattle = (PlayerPrefs.GetInt ("currentBattle")+1);
				//				else
			...
			while (
				...
				if (currentGroup.mode == FightsGroupMode.FightAgainstAllOpponentsInTheGroupInTheDefinedOrder){
					Debug.Log (UFE.storyMode.currentBattle+ "Debug.Log (UFE.storyMode.currentBattle);");
					StoryModeBattle b = currentGroup.opponents[UFE.storyMode.currentBattle];
					Debug.Log (UFE.storyMode.currentBattle);
		...
		if (UFE.storyMode.currentBattleInformation != null){
			...
			//			}
			print ("selected stage: "+UFE.config.selectedStage);
			//UFE.SetStage (UFE.config.selectedStage);
